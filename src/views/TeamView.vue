...
v-for="i in Math.ceil(members?.get(currTeam)?.length / maxMembers)" 
                    :class="'w-[0.5rem] h-[0.5rem] rounded-full border-[#8f8d8b] border-2 ' + (i - 1 == currIdx ? 'bg-[#8f8d8b]' : 'bg-transparent')"></div>
            </div>

            <!-- Team Carousel -->
            <div class="flex justify-center items-center w-[68.5rem] h-[49.6rem]">
                <v-icon :class="'cursor-pointer relative bottom-[3rem] ' + (currIdx == 0 ? 'invisible' : '')" name="ri-arrow-drop-left-line" scale="3.5" fill="#8f8d8b" @click="handleClick(-1)"/>
                
                <!-- Content -->
                <div class="relative h-full w-full overflow-hidden pl-[2.7rem]">
                    <TransitionGroup
:enter-from-class="initialState" :leave-to-class="finalState"
                        :enter-active-class="activeAnimation" 
                        :leave-active-class="activeAnimation" :css="!changingTeam">

                        <div
v-for="i in [currIdx]" 
                            :key="i" class="flex flex-wrap content-start h-full gap-x-[6.6rem] gap-y-[2.47rem]">
                            <ProfileCard
v-for="member in members?.get(currTeam)?.slice(currIdx * maxMembers, (currIdx + 1) * maxMembers)" 
                                :key="member.name + currTeam" :member="member"/>
                        </div>
                    </TransitionGroup>
                </div>

                <v-icon
v-show="currIdx != Math.ceil(members?.get(currTeam)?.length / maxMembers) - 1" class="cursor-pointer relative bottom-[3rem]" name="ri-arrow-drop-right-line" scale="3.5" fill="#8f8d8b"
                    @click="handleClick(1)"/>
            </div>

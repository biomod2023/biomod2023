...
<template>
    <div
        ...
        :class="{'active:opacity-60': $windowWidth < 1024, 'max-h-24': !activated, 'max-h-full': activated}"
        @click="() => activated = !activated"
    >
        <div :class="{'flex justify-between items-center align-middle mr-4': $windowWidth < 1024}">
            <h2 :id="id" ref="title" class="font-title font-medium text-2xl">
                <slot name="title"></slot>
            </h2>
            <svg v-if="$windowWidth < 1024" class="w-8" :class="{'rotate-180 ': activated}" viewBox="0 0 30 17" fill="none" xmlns="http://www.w3.org/2000/svg">
